...
from scipy.signal import cont2discrete, dlti, ss2zpk
...
def mapCtoD(sys_c, t=(0, 1), f0=0.0):
    ...
    **Example:**
    ...
    It can be done as follows::
        ...
        LFc = dlti([[0, 0], [1, 0]], [[1, -1], [0, -1.5]], [[0, 1]], [[0, 0]])
        tdac = [0, 1]
    ...
    if not (type(t) == tuple or type(t) == list) and not (
        type(t[0]) == tuple or type(t[0]) == list
    ):
        ...
        if t1 == 0 and t2 == 1 and D2 == 0:  # No fancy stuff necessary
            ...
            if n2 > 0:
                if t2 == 1:
                    ...
                    Ap[:n, nt - n2 : nt - 1] = Ap[:n, nt - n2 : nt - 1] + np.tile(
                        B2, (1, n2 - 1)
                    )
            ...
            if n2 > 0:
                Cp = Cp + padr(
                    np.hstack((np.zeros((D2.shape[0], n + n1)), D2 * np.ones((1, n2)))),
                    npp,
                )
    sys = (Ap, Bp, Cp, Dp)
    if np.any(~di):
        ...
        for h in range(m):
            for i in range(n):
                if Bc1[i, h] == 0:
                    ...
                    ztf[i, h] = (
                        np.atleast_1d(ABc1[i, h] / Bc1[i, h]),
                        np.array([0.0]),
                        Bc1[i, h],
                    )  # dt = 1
        ...
        for oi in range(n):
            for ii in range(n):
                ...
                stf[oi, ii] = ss2zpk(
                    Ac, np.eye(n), np.eye(n)[oi, :], np.zeros((1, n)), input=ii
                )
