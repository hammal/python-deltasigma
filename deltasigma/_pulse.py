...
def pulse(S, tp=(0.0, 1.0), dt=1.0, tfinal=10.0, nosum=False):
    ...
    **Returns:**
    ...
    for Si in S:
        y2 = None
        for So in Si:
            _, y2i = step(So, T=np.arange(0.0, tfinal + delta_t, delta_t))
            if y2 is None:
                y2 = y2i.reshape((y2i.shape[0], 1, 1))
            else:
                y2 = np.concatenate((y2, y2i.reshape((y2i.shape[0], 1, 1))), axis=1)
        if y1 is None:
            y1 = y2
        else:
    ...
    for i in range(ndac):
        ...
        yy = +np.concatenate(
            (np.zeros(z1), y1[: nf - n1 + 1, :, i * nis : (i + 1) * nis]), axis=0
        ) - np.concatenate(
            (np.zeros(z2), y1[: nf - n2 + 1, :, i * nis : (i + 1) * nis]), axis=0
        )
        yy = yy[::nd, :, :]
        if not nosum:  # Sum the responses due to each input set
            y = y + yy
