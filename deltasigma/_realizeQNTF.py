...
def realizeQNTF(ntf, form="FB", rot=False, bn=0.0):
    ...
    **Parameters:**
    ...
    ntf : dlti object or supported description
    ...
    **Returns:**
    ...
    if form == "PFB" or form == "PFF":
        ...
        if np.any(np.logical_not(imageband_zeros[n_in:])):
            ...
            raise ValueError(
                "Please put the image-band zeros at the end of" + " the ntf zeros"
            )
        if n_im > 0:
            A[n_in, n_in - 1] = 0.0
    ...
    if form == "FB":
        ...
        C = np.hstack((np.zeros((1, order - 1), dtype="complex128"), np.atleast_2d(1)))
        ...
        B0 = np.vstack((np.atleast_2d(1), np.zeros((order - 1, 1), dtype="complex128")))
        B = B0.copy()
        B[-1, 0] = bn
        ...
        if rot == True:
            ABCD = np.vstack((np.hstack((A, B)), np.hstack((C, np.atleast_2d(0)))))
        ...
        C = C[:, ::-1]
        if rot == True:
            ABCD = np.vstack((np.hstack((A, B)), np.hstack((C, np.zeros((1, 1))))))
            for i in range(1, order - 1):
                phi = np.angle(ABCD[-1, i])
                ABCD[i, :] = ABCD[i, :] * np.exp(+1j * phi)
                ABCD[:, i] = ABCD[:, i] * np.exp(-1j * phi)
