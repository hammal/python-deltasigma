...
class TestCalculateTF(unittest.TestCase):
    ...
    def setUp(self):
        ABCD = [
            [1.0, 0.0, 0.0, 0.044408783846879, -0.044408783846879],
            [
                0.999036450096481,
                0.997109907515262,
                -0.005777399147297,
        ...
        self.ntf_zeros, self.mntf_zeros = (cplxpair(ntf_zeros), cplxpair(mntf_zeros))
        self.stf_zeros, self.mstf_zeros = (cplxpair(stf_zeros), cplxpair(mstf_zeros))
        self.ntf_poles, self.mntf_poles = (cplxpair(ntf_poles), cplxpair(mntf_poles))
        self.stf_poles, self.mstf_poles = (cplxpair(stf_poles), cplxpair(mstf_poles))
    ...
    def test_calculateTF2(self):
        ...
        # test an easy TF
        ABCD = np.array([[1.0, 1.0, -1.0], [1.0, 0.0, 0.0]])
        k = 1.0
        ntf, stf = ds.calculateTF(ABCD, k)
        ntf_zeros, ntf_poles, ntf_gain = ntf
        stf_zeros, stf_poles, stf_gain = stf
        self.assertTrue(np.allclose(stf_poles, [0.0], rtol=1e-5, atol=1e-8))
        self.assertTrue(not len(stf_zeros))
        self.assertTrue(np.allclose(stf_gain, 1.0, rtol=1e-5, atol=1e-8))
        self.assertTrue(np.allclose(ntf_poles, [0.0], rtol=1e-5, atol=1e-8))
        self.assertTrue(np.allclose(ntf_zeros, [1.0], rtol=1e-5, atol=1e-8))
        self.assertTrue(np.allclose(ntf_gain, 1.0, rtol=1e-5, atol=1e-8))
    ...
    def test_calculateTF4(self):
        ...
        # stfs
        self.assertTrue(np.allclose(stfs[0][1], [0.0], rtol=1e-5, atol=1e-8))
        self.assertTrue(not len(stfs[0][0]))
        self.assertTrue(np.allclose(stfs[0][2], 1.0, rtol=1e-5, atol=1e-8))
        self.assertTrue(np.allclose(stfs[1][1], [0.0], rtol=1e-5, atol=1e-8))
        self.assertTrue(not len(stfs[1][0]))
        self.assertTrue(np.allclose(stfs[1][2], 1.0, rtol=1e-5, atol=1e-8))
        # e1 to V1
        self.assertTrue(np.allclose(ntfs[0, 0][1], [0.0], rtol=1e-5, atol=1e-8))
        self.assertTrue(np.allclose(ntfs[0, 0][0], [1.0], rtol=1e-5, atol=1e-8))
        self.assertTrue(np.allclose(ntfs[0, 0][2], 1.0, rtol=1e-5, atol=1e-8))
        # e1 to V2
        self.assertTrue(np.allclose(ntfs[1, 0][1], [0.0], rtol=1e-5, atol=1e-8))
        self.assertTrue(not len(ntfs[1, 0][0]))
        self.assertTrue(np.allclose(ntfs[1, 0][2], -1.0, rtol=1e-5, atol=1e-8))
        # e2 to V2
        self.assertTrue(not len(ntfs[1, 1][0]))
        self.assertTrue(not len(ntfs[1, 1][1]))
        self.assertTrue(np.allclose(ntfs[1, 1][2], 1.0, rtol=1e-5, atol=1e-8))
        # e2 to V1
        self.assertTrue(np.allclose(ntfs[0, 1][2], 0.0, rtol=1e-5, atol=1e-8))
