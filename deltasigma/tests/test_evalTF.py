...
from scipy.signal import dlti, tf2zpk
...
class TestEvalTF(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.h4 = ds.evalTF(dlti(tstr2.zeros, tstr2.poles, tstr2.gain), z)
        h5tf = dlti(tstr2.zeros, tstr2.poles, tstr2.gain).to_ss()
        self.h5 = ds.evalTF((h5tf.A, h5tf.B, h5tf.C, h5tf.D), z)
        h6tf = np.vstack(
            (np.hstack((h5tf.A, h5tf.B)), np.hstack((h5tf.C, np.atleast_2d(h5tf.D))))
        )
    ...
    def test_evalTF_first(self):
        ...
        self.assertTrue(
            np.allclose(np.abs(self.h1), np.abs(self.h2), atol=1e-8, rtol=1e-5)
        )
