...
from scipy.signal import dlti, ss2zpk, lfilter
...
class testMultipleQ:
    ...
    def testMultipleQ1(self):
        ...
        res = zpk_multiply(
            v1n,
            (
                ds._utils._get_zpk(v2n)[1],
                ds._utils._get_zpk(v2n)[0],
                1.0 / ds._utils._get_zpk(v2n)[2],
            ),
        )
    ...
    def testMultipleQ2(self):
        ...
        filtM1 = [0.0, 0.0, 0.0, 2.0, -1.0]
        filtM2 = [1.0, -2.0, 1.0]
        ntf_eq = zpk_multiply(self.ntfs[1, 1], self.ntfs[1, 1])
        M = self.nlev[0] - 1
        osr = 64
        f0 = 0.0
        f1, f2 = ds.ds_f1f2(OSR=64, f0=0.0, complex_flag=False)
        delta = 2
        ...
        v2 = vx[1, :]
        # filter and combine
        vf = lfilter(filtM1, [1.0], v1) + lfilter(filtM2, [1.0], v2)
        # compute the spectra
        window = ds.ds_hann(N)
        NBW = 1.5 / N
        ...
        spec0_smoothed = ds.circ_smooth(np.abs(spec0) ** 2.0, 16)
        Snn0 = (
            np.abs(ds.evalTF(ntf_eq, np.exp(2j * np.pi * freq))) ** 2
            * 2
            / 12
            * (delta / M) ** 2
        )
        ...
        spec1_smoothed = ds.circ_smooth(np.abs(spec1) ** 2.0, 16)
        Snn1 = (
            np.abs(ds.evalTF(self.ntfs[0, 0], np.exp(2j * np.pi * freq))) ** 2
            * 2
            / 12
            * (delta / M) ** 2
        )
