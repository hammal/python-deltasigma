...
class Testl1norm(unittest.TestCase):
    ...
    def test_l1norm_1(self):
        ...
        self.assertTrue(
            np.allclose(ds.l1norm(self.num_den_tuple), 2.0, rtol=1e-5, atol=1e-8)
        )

    def test_l1norm_2(self):
        """Test function for l1norm() 2/4"""
        self.assertTrue(
            np.allclose(ds.l1norm(self.zpk_tuple), 2.0, rtol=1e-5, atol=1e-8)
        )

    def test_l1norm_3(self):
        """Test function for l1norm() 3/4"""
        self.assertTrue(
            np.allclose(ds.l1norm(self.ABCD_tuple), 2.0, rtol=1e-5, atol=1e-8)
        )

    def test_l1norm_4(self):
        """Test function for l1norm() 4/4"""
        self.assertTrue(np.allclose(ds.l1norm(self.splti), 2.0, rtol=1e-5, atol=1e-8))
