...
class TestScaleABCD(unittest.TestCase):
    ...
    def setUp(self):
        ...
        b = np.concatenate((np.atleast_1d(b[0]), np.zeros((max(b.shape) - 1,))))
        ...
        ABCD_ref1 = np.array(
            [
                ...
                [0, 0.1581, 1.0, -0.7357, 0, 0],
                [0, 0.1259, 0.7962, 0.4142, 0, 0],
                [0, 0, 0, 0.2973, 1.0, -0.9437],
    ...
    def test_scaleABCD(self):
        ...
        self.assertTrue(np.allclose(umax, self.umax_ref, atol=1e-3, rtol=1e-3))
        print("umax = ", umax)
    ...
    def test_scaleABCD_Q(self):
        ...
        ABCD, umax, S = ds.scaleABCD(ABCD_IN, nlev=9, f=1.0 / 16)
        ...
        ABCD_ref = np.array(
            [
                ...
                [
                    0.0 + 0.0j,
                    0.0 + 0.0j,
                    -12.3424213726332 - 1.41434395098423j,
                    -2.01750453626270 - 1.83077728688503j,
                    0.0 + 0.0j,
                    0.0 + 0.0j,
                ],
            ]
