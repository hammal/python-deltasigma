...
class TestSimulateSNR(unittest.TestCase):
    ...
    def test_simulateSNR_1(self):
        ...
        ABCD_ref = np.array(
            [
                [1.0, -1.6252, 0, 0, -0.0789, 0.0789],
                [1.0, -0.6252, 0, 0, -0.0756, 0.0756],
                [0, 1.0, 1.0, -1.6252, -0.2758, 0.2758],
                [0, 1.0, 1.0, -0.6252, 0.0843, -0.0843],
                [0, 0, 0, 1.0, 1.0, 0],
            ]
        )
        ...
        snr_user, amp_user = ds.simulateSNR(
            dlti(*ntf), osr=osr, amp=amp_user, f0=f0, nlev=nlev
        )
        self.assertTrue(np.allclose(snr_user, snr_user_ref[::10], atol=1e-5, rtol=1e-5))
        self.assertTrue(np.allclose(amp_user, amp_user_ref[::10], atol=1e-5, rtol=1e-5))
    ...
    def test_simulateSNR_2(self):
        ...
        b1_ref = [0.008863535715733, 0.093216950269955, 0.444473912607388, 1.0]
        c1_ref = [1.0, 1.0, 1.0]
